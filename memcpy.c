...
void benchmark_memcpy(size_t size, size_t iterations) {
    ...
    if (memcmp(dest_std, dest_custom, size) != 0) {
        fprintf(stderr, "Erreur: Les résultats de memcpy et _memcpy_avx diffèrent pour la taille %zu\n", size);
    } else {
        printf("Taille: %8zu bytes | memcpy: %8.6f s | _memcpy_avx: %8.6f s | Gain: %.2f%%\n",
...
int main() {
    ...
    printf("Benchmark de memcpy vs _memcpy_avx\n");
    printf("Iterations: %zu\n\n", iterations);
    printf("Taille (bytes) | memcpy (s) | _memcpy_avx (s) | Gain\n");
    printf("---------------------------------------------------------\n");
